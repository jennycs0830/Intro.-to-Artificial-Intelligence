...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        bestError=float("inf")
        #Set up the variable bestError equal to infinite
        for i in range(len(featureVals)):
            weightError=0.0
            for j in range(len(featureVals[i])):
                if featureVals[i][j]<0:
                    weightError+=weights[j]*abs(1-labels[j])
                    #Formula:Error=sum(wight*|hj(Xi)-Yi|)
                else:
                    weightError+=weights[j]*abs(0-labels[j])
            if weightError<bestError:
            #Compare the error of each classifier
                bestError=weightError
                #choose the minimum error one recorded in variable bestError
                bestClf=WeakClassifier(features[i])
                #use bestClf to recorded the best minimum error classifier
